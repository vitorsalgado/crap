var eventManager = require('../infrastructure/event/eventManager');

function CpfHandler() {
    var expressions = [/jedi g cpf/i, /jedi gerar cpf/i];

    this.setUp = function () {
        eventManager.listen('message', function (message) {
            var hasAnyMatch = expressions
                .some(function (x) {
                    return message.match(x);
                });

            if (!hasAnyMatch) {
                return;
            }

            eventManager.dispatch('send', generateNewCpf(true));
        });
    };

    this.setUpHelp = function(){
        eventManager.listen('help', function () {
            var help = '[g cpf]  [gerar cpf]                                  Generate a valid CPF\n\n';
            eventManager.dispatch('send', help);
        });
    };
}

function generateNewCpf(cpfWithDots) {
    var cpf = '';

    var n = 9;

    var n1 = randomize(n);
    var n2 = randomize(n);
    var n3 = randomize(n);
    var n4 = randomize(n);
    var n5 = randomize(n);
    var n6 = randomize(n);
    var n7 = randomize(n);
    var n8 = randomize(n);
    var n9 = randomize(n);

    var d1 = n9 * 2 + n8 * 3 + n7 * 4 + n6 * 5 + n5 * 6 + n4 * 7 + n3 * 8 + n2 * 9 + n1 * 10;

    d1 = 11 - (mod(d1, 11));

    if (d1 >= 10)
        d1 = 0;

    var d2 = d1 * 2 + n9 * 3 + n8 * 4 + n7 * 5 + n6 * 6 + n5 * 7 + n4 * 8 + n3 * 9 + n2 * 10 + n1 * 11;

    d2 = 11 - (mod(d2, 11));

    if (d2 >= 10)
        d2 = 0;

    if (cpfWithDots)
        cpf = '' + n1 + n2 + n3 + '.' + n4 + n5 + n6 + '.' + n7 + n8 + n9 + '-' + d1 + d2;
    else
        cpf = '' + n1 + n2 + n3 + n4 + n5 + n6 + n7 + n8 + n9 + d1 + d2;

    return cpf;
}

function randomize(n) {
    return Math.round(Math.random() * n);
}

function mod(dividendo, divisor) {
    return Math.round(dividendo - (Math.floor(dividendo / divisor) * divisor));
}

module.exports = new CpfHandler();
